...
一个多租户 Node.js 网关，对外暴露 S3 兼容接口，对内把对象正文写入 WebDAV 上游，并把 S3 元数据写入 WebDAV 隐藏状态目录。
...
Multipart part 会先写入 WebDAV 隐藏 staging 路径，complete 时由网关读取 parts 并合并为最终对象。WebDAV 上游没有 S3 原生 compose 能力，因此 complete 成本高于 AWS S3。
...
对象正文仍写入每个 bucket 配置的 `rootPath`。S3 元数据、控制面状态、multipart 会话和版本索引写入隐藏目录：
...
注意：不要把 `/.webdavtos3-system` 暴露给普通 WebDAV 用户编辑。删除或篡改该目录会导致 S3 metadata、版本、tagging、multipart 会话或控制面配置丢失。
...
- `src/s3`：S3 协议解析、鉴权、路由、XML 响应、错误模型、状态与生命周期
...
- 状态目录不是 AWS 管理面数据库，备份和迁移时需要同时保留对象正文目录与 `/.webdavtos3-system`。
